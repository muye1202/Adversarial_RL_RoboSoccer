...
class RoboPlayer(Env):
    ...
    def step(self, action):
        ...
        ### MOVE THE PLAYER TO START FORMATION ###
        if not self.agent.in_kick_off_formation and not self.agent.wm.is_before_kick_off():
            print("enter team formation stage")

            # used to flip x coords for other side
            side_mod = 1
            if self.agent.wm.side == WorldModel.SIDE_R:
                side_mod = -1
            
            self.agent.wm.teleport_to_point((5 * side_mod, 0))
            self.agent.in_kick_off_formation = True
    ...
    def reset(self):
        ...
        self.agent.connect(host=self.host, port=self.port,\
                           teamname=self.teamname, unnum=self.unnum)
