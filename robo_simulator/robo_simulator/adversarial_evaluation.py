...
class Adversarial_Evaluate(Node):
    
    def __init__(self):
        super().__init__("adversarial_evaluation")
        ...
        # load model
        self.actor_model = DDPG_robo(0., 0., 0., 0., num_states=8, flag="attacker")
        self.actor_model.actor_model.load_weights("/home/muyejia1202/Robot_Soccer_RL/nu_robo_agent/trained_model/against_defender/att_v2_actor.h5", by_name=True)
    ...
    def timer_callback(self):
        ...
        if (self.is_dead_ball() or self.is_scored()
            ...
            attacker_pos = np.array([self.robot_pos.x, self.robot_pos.y, self.robot_pos.theta])
            extra_states = np.array([dist_to_ball, player_facing, angle])
            ball_pos = np.array([self.ball_pos.x, self.ball_pos.y])
            attacker_state = np.concatenate((attacker_pos, extra_states, ball_pos))
            attacker_input = tf.expand_dims(tf.convert_to_tensor(attacker_state), 0)
            attacker_action = self.actor_model.actor_model.predict(attacker_input, verbose=0)
            outputs = attacker_action[0] * 140.0
            outputs[0] = tf.clip_by_value(outputs[0], 30., 100.)   # dash power
            outputs[1] = tf.clip_by_value(outputs[1], -140., 140.) # dash direction
...
def main(args=None):
    ...
    robot_pub = Adversarial_Evaluate()
