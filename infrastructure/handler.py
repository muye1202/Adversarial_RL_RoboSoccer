...
class MessageHandler:
    ...
    def _handle_sense_body(self, msg):
        """
        Returns the current state of players (such as speed, neck angle, stamina, etc.)
        """
...
class ActionHandler:
    ...
    def tackle(self, power: float, foul: str):
        """
        Sends tackle command for defending player
        """
        
        msg = "(tackle " + str(power) + str(foul) + ")"
        cmd_type = ActionHandler.CommandType.TYPE_PRIMARY
        cmd = ActionHandler.Command(cmd_type, msg)
